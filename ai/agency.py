...
class Agency:
	def __init__(self, config_file: str, objective: str, table_name: str, task_list: deque = None):
		...
		self.user_embedding_counter = 0
		self.objective = objective
		self.user_feedback = ""
		self.task_list = task_list or deque()
		if len(self.task_list) == 0:
			self.add_task({"task_name": "Develop a task list for objective"})
	...
	def add_user_embedding(self, embedding_data: Dict):
		self.user_embedding_counter += 1
		self.index.upsert([(
			f"user_embedding_{self.user_embedding_counter}",
			self.openai_api.get_ada_embedding(embedding_data)),
				{ "user_embedding_id": self.user_embedding_counter }])

	def run(self, settings: Dict = {}):
		# Check for objective changes
		if settings.get("objective") != self.objective:
			self.objective = settings.get("objective")
			self.add_task({"task_name": "Update task list for new objective"})

		# Check for user feedback changes
		if self.user_feedback != settings.get("user_feedback"):
			self.user_feedback = settings.get("user_feedback")
